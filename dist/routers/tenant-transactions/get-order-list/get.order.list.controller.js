import { GetOrderListService } from './get.order.list.service.js';
import { CustomError } from '../../../lib/utils/custom.error.js';
import { HttpRes } from '../../../lib/constant/http.response.js';
import { ResponseHandler } from '../../../lib/utils/response.handler.js';
import database from '../../../lib/config/prisma.client.js';
export const GetOrderListByTenantController = async (req, res, next) => {
    try {
            ...
            if (Array.isArray(status)) {
                ...
                for (const s of statusFilter) {
                    if (!validStatuses.includes(s)) {
                        throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, 'Invalid status parameter');
                    }
                }
            }
            ...
            if (Array.isArray(category)) {
                ...
                for (const c of categoryFilter) {
                    if (!validCategories.includes(c)) {
                        throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, 'Invalid category parameter');
                    }
                }
            }
        ...
        if (date_from && typeof date_from === 'string') {
            ...
            if (isNaN(dateFrom.getTime())) {
                throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, 'Invalid date_from format. Use YYYY-MM-DD');
            }
        }
        if (date_to && typeof date_to === 'string') {
            dateTo = new Date(date_to);
            if (isNaN(dateTo.getTime())) {
                throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, 'Invalid date_to format. Use YYYY-MM-DD');
            }
        }
        // Validate sort_by
        let sortBy;
        if (sort_by && typeof sort_by === 'string') {
            ...
            if (!validSortFields.includes(sort_by)) {
                throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, 'Invalid sort_by parameter. Valid values: created_at, check_in_date, total_price');
            }
            sortBy = sort_by;
        }
        else {
        ...
        res
            ...
            .json(ResponseHandler.success('Order list retrieved successfully', result));
    }
    catch (error) {
        next(error);
    }
