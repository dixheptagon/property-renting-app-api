...
export const CancelOrderByTenantController = async (req, res, next) => {
    try {
        ...
        const { cancellation_reason } = await CancelOrderByTenantSchema.validate(req.body, {
            abortEarly: false,
        });
        ...
        if (booking.property.user_id !== user.id) {
            throw new CustomError(HttpRes.status.FORBIDDEN, HttpRes.message.FORBIDDEN, 'Access denied. You can only cancel orders for your own properties.');
        }
        // Check if booking is in pending_payment status
        if (booking.status !== 'pending_payment') {
            throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, `Cannot cancel booking. Current status: ${booking.status}. Only pending_payment bookings can be cancelled by tenant.`);
        }
        ...
        res.status(HttpRes.status.OK).json(ResponseHandler.success('Booking cancelled successfully by tenant.', {
