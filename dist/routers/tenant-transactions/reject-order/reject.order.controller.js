...
export const RejectOrderController = async (req, res, next) => {
    try {
        ...
        if (booking.property.user_id !== user.id) {
            throw new CustomError(HttpRes.status.FORBIDDEN, HttpRes.message.FORBIDDEN, 'Access denied. You can only reject orders for your own properties.');
        }
        // Check if booking is in processing status
        if (booking.status !== 'processing') {
            throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, `Cannot reject booking. Current status: ${booking.status}. Only processing bookings can be rejected.`);
        }
        ...
        res.status(HttpRes.status.OK).json(ResponseHandler.success('Booking rejected successfully. Rejection email sent to guest. Guest has 2 hours to upload new payment proof.', {
