...
exports.RejectOrderController = void 0;
...
const send_rejection_service_1 = require("./send.rejection.service");
const reject_order_validation_1 = require("./reject.order.validation");
const RejectOrderController = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    try {
        // Validate request body
        const { rejection_reason } = yield reject_order_validation_1.RejectOrderSchema.validate(req.body, {
            abortEarly: false,
        });
        ...
        if (booking.property.user_id !== user.id) {
            throw new custom_error_1.CustomError(http_response_1.HttpRes.status.FORBIDDEN, http_response_1.HttpRes.message.FORBIDDEN, 'Access denied. You can only reject orders for your own properties.');
        }
        // Check if booking is in processing status
        if (booking.status !== 'processing') {
            throw new custom_error_1.CustomError(http_response_1.HttpRes.status.BAD_REQUEST, http_response_1.HttpRes.message.BAD_REQUEST, `Cannot reject booking. Current status: ${booking.status}. Only processing bookings can be rejected.`);
        }
        ...
        try {
            yield send_rejection_service_1.SendRejectionService.sendBookingRejectionEmail(updatedBooking.id);
        }
        catch (emailError) {
            console.error('Failed to send rejection email:', emailError);
            // Continue with success response, email failure shouldn't block the rejection
        }
        res.status(http_response_1.HttpRes.status.OK).json(response_handler_1.ResponseHandler.success('Booking rejected successfully. Rejection email sent to guest. Guest has 2 hours to upload new payment proof.', {
...
exports.RejectOrderController = RejectOrderController;
