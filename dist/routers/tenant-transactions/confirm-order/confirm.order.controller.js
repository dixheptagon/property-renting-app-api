...
export const ConfirmOrderController = async (req, res, next) => {
    try {
        ...
        if (booking.property.user_id !== user.id) {
            throw new CustomError(HttpRes.status.FORBIDDEN, HttpRes.message.FORBIDDEN, 'Access denied. You can only confirm orders for your own properties.');
        }
        // Check if booking is in processing status
        if (booking.status !== 'processing') {
            throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, `Cannot confirm booking. Current status: ${booking.status}. Only processing bookings can be confirmed.`);
        }
        ...
        try {
            await SendConfirmationService.sendBookingConfirmationEmail(updatedBooking.id);
        }
        catch (emailError) {
            console.error('Failed to send confirmation email:', emailError);
            // Continue with success response, email failure shouldn't block the confirmation
        }
        res.status(HttpRes.status.OK).json(ResponseHandler.success('Booking confirmed successfully. Confirmation email sent to guest.', {
