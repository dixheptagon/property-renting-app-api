...
export const CheckEmailController = async (req, res, next) => {
    try {
        ...
        if (existingUser) {
            return res.status(HttpRes.status.OK).json(ResponseHandler.success(HttpRes.message.CONFLICT +
        ...
        return res.status(HttpRes.status.OK).json(ResponseHandler.success(HttpRes.message.OK + ': Email available for registration.', {
            exists: false,
            email: email,
            canProceed: true,
        }));
