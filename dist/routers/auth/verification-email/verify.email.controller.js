...
import { VerifyOtpSchema, VerifyTokenSchema, } from './verify.email.validation.js';
...
export const VerifyEmailController = async (req, res, next) => {
    try {
        ...
        if (req.query.verification_token) {
            const { verification_token } = await VerifyTokenSchema.validate(req.query, { abortEarly: false });
            whereClause.verification_token = verification_token;
        }
        else {
            // kalau OTP, ambil dari body
            const { email: bodyEmail, verification_code } = await VerifyOtpSchema.validate(req.body, { abortEarly: false });
            email = bodyEmail;
        ...
        if (!emailVerification) {
            throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, 'Invalid or expired verification code/ verification token');
        }
        // Check if expired
        if (!emailVerification.expires_at ||
            emailVerification.expires_at < new Date()) {
            throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, 'Verification code has expired. Please request a new one.');
        }
        // Mark as used
        ...
        return res.status(HttpRes.status.OK).json(ResponseHandler.success('Email verified successfully. Please complete your registration.', {
