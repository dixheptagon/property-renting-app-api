...
export const getPropertyDetailsController = async (req, res, next) => {
    try {
        ...
        res
            ...
            .json(ResponseHandler.success('Property details retrieved successfully', propertyDetails));
    }
    catch (error) {
        if (error instanceof Error && error.message === HttpRes.message.NOT_FOUND) {
            return next(new CustomError(HttpRes.status.NOT_FOUND, HttpRes.message.NOT_FOUND, 'Property not found'));
        }
