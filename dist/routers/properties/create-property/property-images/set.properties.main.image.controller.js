...
export const setPropertiesMainImageController = async (req, res, next) => {
    try {
        ...
        if (isNaN(id)) {
            throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, 'Invalid image ID');
        }
        // Validate that either property_id or temp_group_id is provided
        if (!property_id && !temp_group_id) {
            throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, 'Either property_id or temp_group_id must be provided');
        }
        if (property_id && temp_group_id) {
            throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, 'Cannot provide both property_id and temp_group_id');
        }
        // Validate property_id if provided
        if (property_id) {
            const propertyIdNum = parseInt(property_id, 10);
            if (isNaN(propertyIdNum)) {
                throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, 'Invalid property_id');
            }
        }
        ...
        const result = await database.$transaction(async (tx) => {
            ...
            if (!targetImage) {
                throw new CustomError(HttpRes.status.NOT_FOUND, HttpRes.message.NOT_FOUND, 'Image not found');
            }
            // Verify the image belongs to the specified property or temp group
            if (property_id) {
                const propertyIdNum = parseInt(property_id, 10);
                if (targetImage.property_id !== propertyIdNum) {
                    throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, 'Image does not belong to the specified property');
                }
            }
            else if (temp_group_id) {
                if (targetImage.temp_group_id !== temp_group_id) {
                    throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, 'Image does not belong to the specified temp group');
                }
            }
