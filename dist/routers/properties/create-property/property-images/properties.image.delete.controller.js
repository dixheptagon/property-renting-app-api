...
export const propertyImageDeleteController = async (req, res, next) => {
    const { imageId, temp_group_id } = req.params;
    try {
        // Validate that either imageId or temp_group_id is provided
        if (!imageId && !temp_group_id) {
            throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, 'Either imageId or temp_group_id must be provided');
        }
        ...
        const result = await database.$transaction(async (tx) => {
            ...
            if (id) {
                ...
                if (imagesToDelete.length === 0) {
                    throw new CustomError(HttpRes.status.NOT_FOUND, HttpRes.message.NOT_FOUND, 'No images found for the specified temp_group_id');
                }
            }
            ...
            for (const image of imagesToDelete) {
                ...
                try {
                    ...
                    throw new CustomError(HttpRes.status.INTERNAL_SERVER_ERROR, HttpRes.message.INTERNAL_SERVER_ERROR, 'Invalid image URL format');
                }
