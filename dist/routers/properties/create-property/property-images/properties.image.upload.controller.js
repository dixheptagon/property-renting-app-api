...
export const propertyImageUploadController = async (req, res, next) => {
    const uploadedPublicIds = [];
    try {
        const files = req.files;
        let { temp_group_id } = req.body;
        if (!files || files.length === 0) {
            throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, 'No files uploaded');
        }
        ...
        if (!temp_group_id) {
            ...
            throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, 'temp_group_id must be a string');
        }
        const totalImages = await database.propertyImage.count({
            where: { temp_group_id },
        });
        if (totalImages >= 10) {
            throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, `You have already uploaded ${totalImages} images, you can only upload a maximum of 10 images per property`);
        }
        ...
        const uploadResults = await database.$transaction(async (tx) => {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                if (!allowedMimes.includes(file.mimetype)) {
                    throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, `Invalid file type: ${file.mimetype}. Only JPEG, PNG, JPG, AVIF, and WebP are allowed.`);
                }
                // Validate file size (3MB max)
                if (file.size > 2 * 1024 * 1024) {
                    throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, 'File size must be less than 2MB');
                }
                // Generate UUID for public_id
                const uuid = crypto.randomUUID();
                const publicId = uuid;
                // Upload to Cloudinary with custom public_id
                const uploadResult = (await cloudinaryUploadTempPropertyImage(file.buffer, { public_id: publicId, temp_group_id }));
        ...
        res
            ...
            .json(ResponseHandler.success('Images uploaded successfully', uploadResults));
    }
    catch (error) {
        // Cleanup uploaded Cloudinary images on failure
        if (uploadedPublicIds.length > 0) {
            try {
                await Promise.allSettled(uploadedPublicIds.map((publicId) => cloudinaryDeleteTempPropertyImage(publicId)));
            }
            catch (cleanupError) {
