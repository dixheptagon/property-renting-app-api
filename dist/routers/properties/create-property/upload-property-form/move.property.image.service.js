...
export const movePropertyImagesService = async (propertyId) => {
    return await database.$transaction(async (tx) => {
        try {
            ...
            const propertyImagePromises = propertyImages.map(async (img) => {
                if (!img.public_id) {
                    throw new CustomError(HttpRes.status.INTERNAL_SERVER_ERROR, HttpRes.message.INTERNAL_SERVER_ERROR, 'Property image public_id is null');
                }
            ...
            for (const room of rooms) {
                ...
                const roomImagePromises = roomImages.map(async (img) => {
                    if (!img.public_id) {
                        throw new CustomError(HttpRes.status.INTERNAL_SERVER_ERROR, HttpRes.message.INTERNAL_SERVER_ERROR, 'Room image public_id is null');
                    }
            ...
            throw new CustomError(HttpRes.status.INTERNAL_SERVER_ERROR, HttpRes.message.INTERNAL_SERVER_ERROR, 'Failed to move property images');
        }
    });
};
