...
export const movePropertyImagesController = async (req, res, next) => {
    try {
        // Get propertyId from request params
        const { propertyId } = req.params;
        if (!propertyId || isNaN(Number(propertyId))) {
            throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, 'Valid propertyId is required');
        }
        ...
        if (!property) {
            throw new CustomError(HttpRes.status.NOT_FOUND, HttpRes.message.NOT_FOUND, 'Property not found or does not belong to tenant');
        }
