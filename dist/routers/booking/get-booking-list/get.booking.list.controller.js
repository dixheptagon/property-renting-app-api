...
import { normalizeTimezone } from '../utils/normalized.date.js';
export const GetBookingListController = async (req, res, next) => {
    try {
        ...
        if (status) {
            ...
            if (invalidStatuses.length > 0) {
                throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, `Invalid status(es): ${invalidStatuses.join(', ')}`);
            }
            // Apply status filter
        ...
        if (date_from || date_to) {
            ...
            if (date_from && typeof date_from === 'string') {
                ...
                if (isNaN(fromDate.getTime())) {
                    throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, 'Invalid date_from format');
                }
            ...
            if (date_to && typeof date_to === 'string') {
                ...
                if (isNaN(toDate.getTime())) {
                    throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, 'Invalid date_to format');
                }
        ...
        res
            ...
            .json(ResponseHandler.success('Bookings retrieved successfully', response));
    }
    catch (error) {
        next(error);
    }
