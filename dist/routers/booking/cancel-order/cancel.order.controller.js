...
import { CancelOrderSchema } from './cancel.order.validation.js';
export const CancelOrderController = async (req, res, next) => {
    try {
        ...
        if (booking.status !== 'pending_payment') {
            throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, 'Cancellation not allowed. Booking must be in pending_payment status.');
        }
        ...
        res.status(HttpRes.status.OK).json(ResponseHandler.success('Booking cancelled successfully', {
