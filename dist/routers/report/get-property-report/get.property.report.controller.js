...
export const getPropertyReportController = async (req, res, next) => {
    try {
        ...
        if (user.role !== 'tenant') {
            throw new CustomError(HttpRes.status.FORBIDDEN, HttpRes.message.FORBIDDEN, 'Only tenants can access property reports');
        }
        ...
        if (!room_type_uid || typeof room_type_uid !== 'string') {
            throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, 'Room type UID is required and must be a string');
        }
        if (!selected_date || typeof selected_date !== 'string') {
            throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, 'Selected date is required and must be a string (YYYY-MM-DD format)');
        }
        ...
        if (room_type_uid && typeof room_type_uid === 'string') {
            ...
            if (!room) {
                throw new CustomError(HttpRes.status.NOT_FOUND, HttpRes.message.NOT_FOUND, 'Room type not found');
            }
            if (room.property_id !== propertyId) {
                throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, 'Room type does not belong to the specified property');
            }
        ...
        if (totalUnits === 0) {
            ...
            return res
                ...
                .json(ResponseHandler.success('Property report retrieved successfully', result));
        }
