import { CreateReviewSchema } from './create.review.validation.js';
...
export const CreateReviewController = async (req, res, next) => {
    try {
        ...
        if (booking.user_id !== user.id) {
            throw new CustomError(HttpRes.status.FORBIDDEN, HttpRes.message.FORBIDDEN, 'You can only review your own bookings');
        }
        // Check if booking status is completed
        if (booking.status !== 'completed') {
            throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, 'You can only review completed bookings');
        }
        // Check if current date is after checkout date
        const currentDate = new Date();
        const checkoutDate = new Date(booking.check_out_date);
        if (currentDate <= checkoutDate) {
            throw new CustomError(HttpRes.status.BAD_REQUEST, HttpRes.message.BAD_REQUEST, 'You can only review after the checkout date');
        }
        // Check if review already exists for this booking
        if (booking.review) {
            throw new CustomError(HttpRes.status.CONFLICT, HttpRes.message.CONFLICT, 'Review already exists for this booking');
        }
        // Create review
        ...
        res
            ...
            .json(ResponseHandler.success('Review created successfully', responseData));
    }
    catch (error) {
        next(error);
    }
