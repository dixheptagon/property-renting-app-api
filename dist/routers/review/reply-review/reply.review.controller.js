...
export const ReplyReviewController = async (req, res, next) => {
    try {
        const { booking_uid, reply_comment } = await ReplyReviewSchema.validate(req.body, {
            abortEarly: false,
        });
        ...
        if (!booking.review) {
            throw new CustomError(HttpRes.status.NOT_FOUND, HttpRes.message.NOT_FOUND, 'Review not found for this booking');
        }
        // Check if tenant owns the property
        if (booking.property.user_id !== user.id) {
            throw new CustomError(HttpRes.status.FORBIDDEN, HttpRes.message.FORBIDDEN, 'You can only reply to reviews for properties you own');
        }
        ...
        res.status(HttpRes.status.OK).json(ResponseHandler.success('Reply added to review successfully', {
            id: updatedReview.id,
