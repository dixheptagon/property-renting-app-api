...
export const GetPropertyListByTenantController = async (
  ...
  try {
    ...
    if (status && typeof status === 'string') {
      ...
      if (!validStatuses.includes(status.toLowerCase())) {
        throw new CustomError(
          ...
          'Invalid status parameter. Valid values: draft, active, deleted',
        );
      }
    ...
    if (sort_by && typeof sort_by === 'string') {
      ...
      if (!validSortFields.includes(sort_by)) {
        throw new CustomError(
          HttpRes.status.BAD_REQUEST,
          HttpRes.message.BAD_REQUEST,
          'Invalid sort_by parameter. Valid values: created_at, updated_at, title, base_price',
        );
      }
      sortBy = sort_by;
    } else {
    ...
    res
      ...
      .json(
        ResponseHandler.success('Property list retrieved successfully', result),
      );
  } catch (error) {
    next(error);
  }
