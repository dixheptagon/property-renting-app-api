  ...
  try {
    ...
    if (!isSignatureValid) {
      throw new CustomError(
        ...
        `Invalid Midtrans signature for order_id: ${order_id}`,
      );
    }
    ...
    if (!booking) {
      ...
      throw new CustomError(
        HttpRes.status.OK,
        HttpRes.message.OK,
        `Booking not found for order_id: ${order_id}`,
      );
    }
    ...
    await database.$transaction(async (tx) => {
      if (
        ...
        console.log(
          'Transaction status is expire/cancel/deny, booking status:',
          booking.status,
        );
        // Only update if still pending_payment
        if (booking.status === 'pending_payment') {
          ...
          console.log(
            'Booking status not pending_payment, skipping cancellation update',
          );
        }
      } else {
      ...
      res
        ...
        .json(
          ResponseHandler.success(
            HttpRes.message.OK + ' : Transaction Successfull',
            'Check your inbox and email for more details',
          ),
        );
  } catch (error) {
